...
then
    ...
    let "noBackups+=1"
    cd ..
    echo "$(date '+%Y-%m-%d-%H-%M-%S'): Backup process has been initiated... initial backup executed"
fi
...
do
    ...
    then
        echo "$(date '+%Y-%m-%d-%H-%M-%S'): Files have not been changed since last backup... No backup has been executed"
    else
        while (( $noBackups>=$max ))
        do
            cd $backupdir
        ...
        cd ..
        ls -lR $dir > $backupdir/directory-info.last
        echo "$(date '+%Y-%m-%d-%H-%M-%S'): Change detected... new backup has been executed"
    fi
done